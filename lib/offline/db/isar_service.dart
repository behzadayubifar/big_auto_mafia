...
// {name : code}
  ...
  Future<Room> putRoom({
    ...
    if (roomExists != null) {
      await isar.writeTxn(() => isar.rooms.put(roomExists.copy(
            id: id,
            name: name,
  ...
  Future<Room?> retrieveRoomByID(String id) async {
    final room = await isar.rooms.filter().idEqualTo(id).findFirst();
    if (room != null) {
      return room;
    }
    return null;
  ...
  Future<bool> deleteRoom(String id) async {
    ...
    if (roomToDelete != null) {
      await isar.writeTxn(() => isar.rooms.delete(roomToDelete.isarId));
